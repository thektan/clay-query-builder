...
class ClayCriteriaRow extends React.Component {
	render() {
		...
		const selectedProperty = this._getSelectedItem(properties, criterion.propertyName);
						...
						{editing ? (
							<div className="form-group-autofit">
								<div className="form-group-item">
									<select className="criteria" value={selectedProperty.name} onChange={this._handleInputChange('propertyName')}>
										{properties.map((property, index) => (
											<option value={property.name} key={index}>{property.label}</option>
										))}
									</select>
								</div>

								<div className="form-group-item">
									<select className="operator" value={selectedOperator.name} onChange={this._handleInputChange('operatorName')}>
										{operators.map((operator, index) => (
											<option value={operator.name} key={index}>{operator.label}</option>
										))}
									</select>
								</div>

								<div className="form-group-item">
									<input className="form-control" id="queryRowValue" value={criterion.value} onChange={this._handleInputChange('value')} type="text"/>
								</div>
							...
							<div className="query-row-display">
								<div className="query">
									<span className="criteria">{selectedProperty.label}</span>

									<span className="operator">{selectedOperator.label}</span>

									<span className="value">{criterion.value}</span>
								</div>
							</div>
						)}
					</div>
